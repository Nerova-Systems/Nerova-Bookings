...
4. Use appropriate PostgreSQL data types:
   ...
   - Use `jsonb` for all JSON columns--both `OwnsOne(..., b => b.ToJson())` and manually serialized collections/objects using `HasConversion` with `JsonSerializer`. When using `HasConversion`, also add `.HasColumnType("jsonb")` in the EF configuration so Npgsql sends the value as `jsonb` instead of `text`
   - Enforce length constraints at the application level using FluentValidation, not at the database level

5. For indexes on large tables, use `migrationBuilder.Sql("CREATE INDEX CONCURRENTLY ...")` to avoid locking the table during creation. This is safe because the CI/CD pipeline applies migrations with `ON_ERROR_STOP=1` without `--single-transaction`, and the generated script is idempotent.

6. Create appropriate constraints and indexes:
