...
public sealed record PortAllocation(int BasePort)
{
    ...
    public static PortAllocation LoadFrom(string repositoryRoot)
    {
        ...
        if (!File.Exists(portFilePath))
        {
            ...
            catch (IOException) when (File.Exists(portFilePath))
            {
                // Another test host/process created the allocation between File.Exists and WriteAllText.
            }
        }

        var content = ReadPortFileWithRetry(portFilePath).Trim();
    ...
    private static string ReadPortFileWithRetry(string portFilePath)
    {
        ...
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            try
            {
                return File.ReadAllText(portFilePath);
            }
