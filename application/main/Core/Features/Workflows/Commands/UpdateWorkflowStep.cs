...
public sealed class UpdateWorkflowStepValidator : AbstractValidator<UpdateWorkflowStepCommand>
{
    public UpdateWorkflowStepValidator()
    {
...
public sealed class UpdateWorkflowStepHandler(
    IWorkflowRepository workflowRepository,
    IExecutionContext executionContext,
    ITelemetryEventsCollector events
) : IRequestHandler<UpdateWorkflowStepCommand, Result<WorkflowStepResponse>>
{
    public async Task<Result<WorkflowStepResponse>> Handle(UpdateWorkflowStepCommand command, CancellationToken cancellationToken)
    {
        ...
        var updated = workflow.UpdateStep(command.StepId, command.Action, command.Template, command.ReminderTime, command.TimeUnit, command.SendTo, command.EmailSubject, command.EmailBody);
        if (!updated)
        {
            return Result<WorkflowStepResponse>.NotFound($"Step '{command.StepId}' was not found in workflow '{command.WorkflowId}'.");
        }

        var step = workflow.Steps.First(s => s.Id == command.StepId);
        events.CollectEvent(new WorkflowStepUpdated(workflow.Id, step.Id));
