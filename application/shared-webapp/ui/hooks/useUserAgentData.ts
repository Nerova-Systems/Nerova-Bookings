...
/**
 * Detects the user's operating system and browser on the client.
 * Returns `{ os, browser, isMobile }` — all fields default to `"unknown"` / `false`
 * during SSR and on mount.
 *
 * Ported from cal.com `packages/lib/hooks/useUserAgentData.ts` (cf2a55c).
 */
export function useUserAgentData(): UserAgentData {
  ...
  useEffect(() => {
    ...
    setUserAgentData({ os, browser, isMobile: os === "ios" || os === "android" });
  }, []);
