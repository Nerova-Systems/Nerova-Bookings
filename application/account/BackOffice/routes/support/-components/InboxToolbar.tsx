...
export function InboxToolbar({ search, category, assignee, resultCount }: Readonly<InboxToolbarProps>) {
  ...
  const handleCategoryChange = (value: string) => {
    ...
    navigate({
      to: "/support/tickets",
      search: (previous) => ({ ...previous, category: next, pageOffset: undefined })
    });
  };
  ...
  const handleAssigneeChange = (value: SupportTicketAssigneeFilter | undefined) => {
    navigate({
      to: "/support/tickets",
      search: (previous) => ({ ...previous, assignee: value, pageOffset: undefined })
    });
  };
      ...
      <div className="max-w-[22rem] min-w-[14rem] flex-1">
        <InputGroup>
          ...
          <InputGroupInput
            ...
            role="searchbox"
            aria-label={t`Search`}
            placeholder={t`Search subject, reporter, or ID`}
            value={searchInput}
