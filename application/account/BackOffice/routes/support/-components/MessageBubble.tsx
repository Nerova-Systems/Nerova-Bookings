...
export function MessageBubble({ message }: { message: StaffMessage }) {
  ...
  return (
    <div className={`flex items-start gap-2.5 ${isStaffSide ? "flex-row-reverse" : ""}`}>
      <Avatar size="lg" className="shrink-0">
        <AvatarFallback className={isStaffSide ? "bg-primary/10 text-primary" : undefined}>{initials}</AvatarFallback>
      </Avatar>
              ...
              {message.attachments.map((attachment) => (
                <a
                  ...
                  className={`inline-flex items-center gap-2 rounded-lg border px-2.5 py-1.5 text-xs underline-offset-2 hover:underline ${
                    isStaff ? "border-background/40 text-background" : "border-border text-foreground"
                  }`}
