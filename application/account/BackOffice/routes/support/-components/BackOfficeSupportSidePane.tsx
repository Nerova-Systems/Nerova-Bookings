    ...
    >
      ...
      {ticket && (
        <SidePaneFooter className="flex flex-col gap-3 border-t border-border bg-card">
          <AssigneeBlock assignee={ticket.assignee} />
...
function AssigneeBlock({ assignee }: { assignee: Schemas["StaffTicketAssignee"] | null }) {
  if (!assignee) return null;
  return (
    <div className="flex items-center gap-2">
      <Avatar size="default" className="size-7">
        <AvatarFallback className="bg-primary/10 text-primary">{getInitials(assignee.displayName)}</AvatarFallback>
      </Avatar>
      <div className="flex min-w-0 flex-col">
        <span className="text-xs text-muted-foreground">
          <Trans>Assigned to</Trans>
        </span>
        <span className="truncate text-sm font-medium">{assignee.displayName}</span>
      </div>
    </div>
  );
}
