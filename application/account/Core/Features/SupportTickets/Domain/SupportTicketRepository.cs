...
public interface ISupportTicketRepository : ICrudRepository<SupportTicket, SupportTicketId>
{
    ...
    /// <summary>
    ///     Returns whether the proposed six-character short display ID is unused tenant-wide.
    ///     Display IDs are unique within a tenant; collisions are extremely unlikely but possible.
    /// </summary>
    Task<bool> IsShortDisplayIdFreeForTenantAsync(string shortDisplayId, CancellationToken cancellationToken);
    ...
    : RepositoryBase<SupportTicket, SupportTicketId>(accountDbContext), ISupportTicketRepository
{
    ...
    public async Task<bool> IsShortDisplayIdFreeForTenantAsync(string shortDisplayId, CancellationToken cancellationToken)
    {
        return !await DbSet.AnyAsync(t => t.ShortDisplayId == shortDisplayId, cancellationToken);
    }
