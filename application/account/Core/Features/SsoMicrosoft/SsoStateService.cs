...
namespace Account.Features.SsoMicrosoft;

public sealed record SsoStateCookie(TenantId OrgId, string FingerprintHash, string CodeVerifier, string Nonce);

/// <summary>
///     Manages the <c>__Host-sso-microsoft-state</c> cookie used to carry SSO flow state
...
/// </summary>
public sealed class SsoStateService(IHttpContextAccessor httpContextAccessor, IDataProtectionProvider dataProtectionProvider, ILogger<SsoStateService> logger)
{
    private const string DataProtectionPurpose = "SsoLogin.Microsoft";
    private const string CookieName = "__Host-sso-microsoft-state";
    ...
    {
        return $"{PublicUrl}/api/account/sso/microsoft/callback";
    }
    ...
    /// </summary>
    public SsoStateCookie? GetStateCookie()
    {
        ...
        try
        {
            ...
            return new SsoStateCookie(orgId, parts[1], parts[2], parts[3]);
        }
        catch (Exception ex)
        {
            logger.LogWarning(ex, "Failed to decrypt SSO state cookie");
            return null;
        }
    }
    ...
    ///     browser fingerprint. See <c>ExternalAuthenticationService.GenerateBrowserFingerprintHash</c>
    ///     for documented limitations of this approach.
    /// </summary>
    ...
    /// <summary>Extracts the org tenant ID from the CSRF state token returned by Microsoft.</summary>
    public TenantId? GetOrgIdFromStateToken(string? state)
    {
        ...
        {
            logger.LogWarning(ex, "Failed to decrypt SSO state token");
            return null;
        }
    }
}
