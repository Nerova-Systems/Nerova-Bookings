...
test.describe("@smoke", () => {
  ...
  test("should handle complete subscription lifecycle with plan changes and billing states", async ({ ownerPage }) => {
    ...
    await step("Submit billing info & verify Paystack hosted checkout redirect starts")(async () => {
      ...
      await ownerPage.route("https://checkout.paystack.com/**", async (route) => {
        await route.fulfill({
          status: 200,
          contentType: "text/html",
          body: "<!doctype html><title>Paystack Checkout</title>"
        });
      });
      const checkoutRedirect = ownerPage.waitForURL(/checkout\.paystack\.com/);

      await ownerPage.getByRole("button", { name: "Next" }).click();

      await checkoutRedirect;
      await ownerPage.unroute("https://checkout.paystack.com/**");
    })();
    ...
    await step("Mock Standard subscription & click Upgrade on Premium plan & confirm upgrade dialog")(async () => {
      ...
      await ownerPage.route("**/api/account/subscriptions/upgrade-preview**", async (route) => {
        await route.fulfill({
          ...
          json: {
            lineItems: [
              { description: "Premium (prorated)", amount: 70.0, currency: "ZAR", isTax: false },
      ...
      await ownerPage.route("**/api/account/subscriptions/upgrade", async (route) => {
        ...
        await route.fulfill({
          status: 200,
          contentType: "application/json",
          json: { authorizationUrl: null, reference: null }
        });
      });
