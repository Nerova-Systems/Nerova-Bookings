...
export function useTeamMemberPicker({ teamId, isOpen, onClose }: UseTeamMemberPickerOptions) {
  ...
  const updateMembersMutation = api.useMutation("put", "/api/account/teams/{id}/members", {
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["get", "/api/account/teams/{id}/members"] });
  ...
  const originalMembers = membersQuery.data;
  ...
  const save = () => {
    ...
    updateMembersMutation.mutate({ params: { path: { id: teamId } }, body });
  };

  return {
    search,
    setSearch,
    isLoading: membersQuery.isLoading || usersQuery.isLoading,
    availableUsers,
    teamMembers,
    selectedAvailable,
    selectedMembers,
