...
public sealed class GetTenantsTests(BackOfficeWebApplicationFactory factory) : BackOfficeEndpointBaseTest(factory), IClassFixture<BackOfficeWebApplicationFactory>
{
    [Fact]
    public async Task GetTenants_WhenCalled_ShouldReturnAllTenantsWithSummaryFields()
    {
        // Arrange
        SeedTenant("Acme Corp", SubscriptionPlan.Standard, 49.99m, MockPaystackClient.MockStandardCurrency, "US", false, null, 30);
    ...
    {
        // Arrange
        var tenantA = SeedTenant("Acme Corp", SubscriptionPlan.Standard, 49.99m, MockPaystackClient.MockStandardCurrency, "US", false, null, 30);
    ...
    {
        // Arrange
        var tenantA = SeedTenant("Acme Corp", SubscriptionPlan.Standard, 49.99m, MockPaystackClient.MockStandardCurrency, "US", false, null, 50);
        SeedTenant("Beta Industries", SubscriptionPlan.Premium, 199.00m, "EUR", "DE", true, null, 40);
        SeedTenant("Cyrus Co", SubscriptionPlan.Standard, 49.99m, MockPaystackClient.MockStandardCurrency, "US", false, SubscriptionPlan.Basis, 30);
    ...
    public async Task GetTenants_WhenFilteringByDowngradingStatus_ShouldReturnOnlyTenantsWithScheduledPlanChange()
    {
        // Arrange
        SeedTenant("Acme Corp", SubscriptionPlan.Standard, 49.99m, MockPaystackClient.MockStandardCurrency, "US", false, null, 30);
        var tenantC = SeedTenant("Cyrus Co", SubscriptionPlan.Standard, 49.99m, MockPaystackClient.MockStandardCurrency, "US", false, SubscriptionPlan.Basis, 20);
        var identity = MockEasyAuthIdentities.Default.Single(i => i.Id == "user");
        using var client = CreateBackOfficeClientForIdentity(identity);
    ...
    private TenantId SeedTenant(string name, SubscriptionPlan plan, decimal? mrr, string? currency, string? country, bool cancelAtPeriodEnd, SubscriptionPlan? scheduledPlan, int createdMinutesAgo, bool hasEverSubscribed = false)
    {
        ...
        Connection.Insert("subscriptions", [
                ...
                ("paystack_customer_code", mrr is null ? null : "cus_test"),
                ("paystack_authorization_code", mrr is null ? null : "sub_test"),
