...
public sealed class GetDashboardKpisTests : BackOfficeEndpointBaseTest
{
    ...
    )
    {
        ...
        var paymentTransactionsJson = JsonSerializer.Serialize(new[]
            {
                new PaymentTransaction(PaymentTransactionId.NewId(), currentPriceAmount, currentPriceAmount, 0m, MockPaystackClient.MockStandardCurrency, PaymentTransactionStatus.Succeeded, subscriptionCreatedAt, null, null, null, plan)
            }
        );
        ...
        Connection.Insert("subscriptions", [
                ...
                ("paystack_authorization_code", "sub_test"),
    ...
    private void SeedSubscription(TenantId tenantId, SubscriptionPlan plan, decimal? currentPriceAmount, bool hasSucceededPayment)
    {
        var paymentTransactionsJson = hasSucceededPayment
            ? JsonSerializer.Serialize(new[]
                {
                    new PaymentTransaction(PaymentTransactionId.NewId(), 49.99m, 49.99m, 0m, MockPaystackClient.MockStandardCurrency, PaymentTransactionStatus.Succeeded, DateTimeOffset.UtcNow.AddDays(-30), null, null, null, SubscriptionPlan.Standard)
                }
            )
            : "[]";
        ...
        Connection.Insert("subscriptions", [
                ...
                ("plan", plan.ToString()),
                ("scheduled_plan", null),
                ("paystack_customer_code", currentPriceAmount is null ? null : "cus_test"),
                ("paystack_authorization_code", currentPriceAmount is null ? null : "sub_test"),
                ("current_price_amount", (object?)currentPriceAmount ?? DBNull.Value),
                ("current_price_currency", currentPriceAmount is null ? null : MockPaystackClient.MockStandardCurrency),
