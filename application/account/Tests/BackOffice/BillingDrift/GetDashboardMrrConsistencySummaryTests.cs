...
public sealed class GetDashboardMrrConsistencySummaryTests : BackOfficeEndpointBaseTest
{
    ...
    [Fact]
    public async Task GetDashboardMrrConsistencySummary_WhenPaystackSubscriptionHasNoBillingEvent_ShouldUseSubscriptionSnapshot()
    {
        // Arrange
        var tenantId = SeedTenant("Paystack Snapshot Co");
        var subscriptionId = SubscriptionId.NewId();
        SeedPaidSubscription(tenantId, subscriptionId, 800m, false, null);
        ...
        var payload = await response.Content.ReadFromJsonAsync<DashboardMrrConsistencySummaryResponse>();
        payload.Should().NotBeNull();
        payload.KpiMonthlyRecurringRevenue.Should().Be(800m);
        payload.TrendLatestMonthlyRecurringRevenue.Should().Be(800m);
    }
